...
mHashPtsChk = msg2mset(msgBlocksChk,N);
for i = 1:length(mHashPtsChk)
    ...
    %mHashesChk(i).sig = mandelfn(mHashPtsChk(i),pubEKey_B,priDKey_A.c,priDKey_A.x);
    %mHashesChk(i).sec = mandelfn(pubEKey_B,mHashes(i).sig,priDKey_A.c,priDKey_A.x);
end
...
fprintf('mHashes.sig:\n---------\n');
%fprintf('%.20f\n       * i%.20f\n',mHashes(:).sig);
format long eng, mHashes.sig;
mHashes(:).sig
fprintf('\n-----------------\nAlice Re-Hash:\n---------------------------\n')
fprintf('msgBlocksChk:\n---------\n');
msgBlocksChk{:}
fprintf('mHashPtsChk:\n---------\n');
%fprintf('%.20f\n       * i%.20f\n',mHashPtsChk(:));
mHashPtsChk(:)
...
%fprintf('%.20f\n       * i%.20f\n',mHashesChk(:).sig);
mHashesChk(:).sig
fprintf('mHashesChk.sec:\n---------\n');
%fprintf('%.20f\n       * %.20f\n',mHashesChk(:).sec);
mHashesChk(:).sec

save ./MandelHasher/temp/hashTempA mHashesChk pubEKey_A
...
% Bill Makes secHashes from Alice sigHashes
for i = 1:length(mHashes)
    mHashes(i).sec = mandelfn(globProf.c,mHashesChk(i).sig,priDKey_B.c,priDKey_B.x);
    %mHashes(i).sec = mandelfn(pubEKey_A,mHashesChk(i).sig,priDKey_B.c,priDKey_B.x);
    diffVals(i) = mHashesChk(i).sec - mHashes(i).sec;
    valsAvg(i) = (mHashesChk(i).sec + mHashes(i).sec)/2;
    errMargin(i) = abs(diffVals(i)/valsAvg(i));
end
